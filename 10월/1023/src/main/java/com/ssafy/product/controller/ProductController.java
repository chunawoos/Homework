...
public class ProductController {
	...
	public String regist(Product product, Model model) {
		try {
			...
			return "error/error"; // WEB-INF/views/error/error
		}
		return "redirect:/list"; // return은 이동과 관련있음!!
					   // 이동의 방법 			: 1) forward         2) redirect
					   // 스프링에서 표현법		 return "list"	      return "redirect:/list"
					   // 				   =>list.jsp 페이지 표현      => 정의된 mapping "/list" 찾기
	}

	@RequestMapping(value = "/list", method= RequestMethod.GET)
	public String list(Model model) {
		try {
			model.addAttribute("list",service.searchAll()); // 키값이 생략!! searchAll의 리턴타입 : List
	...
	public String read(String id, Model model) {
		try {
			System.out.println(service.search(id).getDescription());
